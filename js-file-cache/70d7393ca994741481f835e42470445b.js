!function (A, e) {
  "function" == typeof define && define.amd ? define(e) : A.logo = cc11001100_hook("A.logo", e(), "assign");
}("undefined" != typeof self ? self : this, function () {
  function A() {}
  function e() {
    var A = cc11001100_hook("A", null, "var-init");
    try {
      A = cc11001100_hook("A", window.baidu && window.baidu.mobads && window.baidu.mobads.Sdk || window.parent && parent.baidu && parent.baidu.mobads && parent.baidu.mobads.Sdk, "assign");
    } catch (e) {}
    return A;
  }
  function t(A, e) {
    cc11001100_hook("A", A, "function-parameter");
    cc11001100_hook("e", e, "function-parameter");
    var t = cc11001100_hook("t", document.createElement("div"), "var-init");
    t.innerHTML = cc11001100_hook("t.innerHTML", e, "assign");
    for (var n = cc11001100_hook("n", document.createDocumentFragment(), "var-init"); t.childNodes.length > 0;) n.appendChild(t.childNodes[0]);
    A.appendChild(n);
  }
  function n(A, e, t) {
    cc11001100_hook("A", A, "function-parameter");
    cc11001100_hook("e", e, "function-parameter");
    cc11001100_hook("t", t, "function-parameter");
    A.addEventListener ? A.addEventListener(e, t, !1) : A.attachEvent && A.attachEvent("on" + e, t);
  }
  function a(A) {
    cc11001100_hook("A", A, "function-parameter");
    return A.stopPropagation ? A.stopPropagation() : window.event.cancelBubble = cc11001100_hook("window.event.cancelBubble", !0, "assign"), A.preventDefault && A.preventDefault(), !1;
  }
  function o(A) {
    cc11001100_hook("A", A, "function-parameter");
    var e = cc11001100_hook("e", null, "var-init"),
      t = cc11001100_hook("t", [], "var-init");
    for (e in A) A.hasOwnProperty(e) && t.push(e + "=" + encodeURIComponent(A[e]));
    return t.join("&");
  }
  function i(A, e) {
    cc11001100_hook("A", A, "function-parameter");
    cc11001100_hook("e", e, "function-parameter");
    var t = cc11001100_hook("t", e.type, "var-init");
    if ("treatment" === t) {
      var n = cc11001100_hook("n", A.querySelector(".logo-container"), "var-init");
      n.style.cssText = cc11001100_hook("n.style.cssText", "opacity:0.5", "assign");
    }
  }
  function s(A, e) {
    cc11001100_hook("A", A, "function-parameter");
    cc11001100_hook("e", e, "function-parameter");
    A.style.position = cc11001100_hook("A.style.position", "relative", "assign");
    var t = cc11001100_hook("t", A.querySelector(".logo-container"), "var-init"),
      n = cc11001100_hook("n", t.getAttribute("class"), "var-init");
    n = cc11001100_hook("n", n.concat(" new-logo-container"), "assign"), t.setAttribute("class", n), t.style.cssText = cc11001100_hook("t.style.cssText", "background: transparent;", "assign"), t.style.width = cc11001100_hook("t.style.width", "100%", "assign");
    var a = cc11001100_hook("a", A.querySelector(".logo-con-wrapper"), "var-init");
    a.style.cssText = cc11001100_hook("a.style.cssText", "display: inline-block; height: 16px; background: transparent", "assign");
    var o = cc11001100_hook("o", A.querySelector(".logo-close-img"), "var-init"),
      i = cc11001100_hook("i", A.querySelector(".logo-link"), "var-init");
    e ? (o.src = cc11001100_hook("o.src", "data:image/png;base64,", "assign"), i.src = cc11001100_hook("i.src", "data:image/png;base64,", "assign"), o.style.cssText = cc11001100_hook("o.style.cssText", "position: absolute; right: 0; margin: 0; padding: 4px; opacity: 0.3; background: #000000;", "assign"), a.style.background = cc11001100_hook("a.style.background", "#000000", "assign"), a.style.opacity = cc11001100_hook("a.style.opacity", "0.3", "assign")) : (o.src = cc11001100_hook("o.src", "https://render-server.cdn.bcebos.com/static/images/20210525/d5443f3a0c9fe8456beb5f50eb08ed60.png", "assign"), i.src = cc11001100_hook("i.src", "https://render-server.cdn.bcebos.com/static/images/spring_festival/ad-logo.png", "assign"), o.style.cssText = cc11001100_hook("o.style.cssText", "position: absolute; right: 0; margin: 0; padding: 4px; width: 10px; height: 10px", "assign"), i.style.cssText = cc11001100_hook("i.style.cssText", "width: 32px;height: 11px", "assign"));
  }
  function c(A) {
    cc11001100_hook("A", A, "function-parameter");
    A = cc11001100_hook("A", A || {}, "assign");
    var e = cc11001100_hook("e", A.logoUrl || "https://union.baidu.com", "var-init"),
      n = cc11001100_hook("n", document.getElementById(A.containerId || "container"), "var-init"),
      a = cc11001100_hook("a", (A.containerId, document.getElementById(A.feedbackParentId || "container")), "var-init"),
      o = cc11001100_hook("o", !A.hideClose && a, "var-init"),
      c = cc11001100_hook("c", A.closeDirect, "var-init"),
      l = cc11001100_hook("l", A.deviceType > 1 ? " logo-mobile-container" : "", "var-init"),
      v = cc11001100_hook("v", A.exps || {}, "var-init"),
      u = cc11001100_hook("u", A.newLogoExps, "var-init"),
      w = cc11001100_hook("w", v.rs_logo || {}, "var-init"),
      f = cc11001100_hook("f", A.isDarkLogo || !1, "var-init"),
      m = cc11001100_hook("m", A.cloudConfiglogo || !1, "var-init");
    if (null !== n) {
      var O = cc11001100_hook("O", '<img class="logo-link" src="data:image/png;base64," />', "var-init");
      m && (O = cc11001100_hook("O", '<img class="logo-link" style="width:51px;" src="data:image/png;base64," />', "assign"));
      var h = cc11001100_hook("h", ['<div class="logo-container' + l + '">', '<a href="' + e + '" target="_blank" class="logo-con-wrapper">', O, "</a>", g(o), "</div>"].join(""), "var-init");
      if (i(n, w), t(n, h), r(n), u && s(n, f), o) {
        var C = cc11001100_hook("C", d(a.clientWidth, a.clientHeight), "var-init");
        t(a, C), p(A.trackingInfo, c, n), b();
      }
    }
  }
  function r(A) {
    cc11001100_hook("A", A, "function-parameter");
    try {
      var e = cc11001100_hook("e", /ltu=.*(youth.cn)/, "var-init"),
        t = cc11001100_hook("t", window.location.href, "var-init");
      if (e.test(t)) {
        var n = cc11001100_hook("n", A.getElementsByClassName("logo-container")[0], "var-init");
        n.style.cssText = cc11001100_hook("n.style.cssText", "opacity: 1; background-color: #b2b2b2", "assign");
      }
    } catch (a) {}
  }
  function g(A) {
    cc11001100_hook("A", A, "function-parameter");
    return A ? ['<div class="gap"></div>', '<a href="javascript:;" id="logo-close-btn">', '<img class="logo-close-img" ', 'src="data:image/png;base64," />', "</a>"].join("") : "";
  }
  function d(A, e) {
    cc11001100_hook("A", A, "function-parameter");
    cc11001100_hook("e", e, "function-parameter");
    var t = cc11001100_hook("t", e - 250 > 0 ? 30 : 0, "var-init"),
      n = cc11001100_hook("n", 320, "var-init");
    320 > A && A > 159 ? n = cc11001100_hook("n", 160, "assign") : 160 > A && (n = cc11001100_hook("n", 80, "assign"));
    var a = cc11001100_hook("a", 160 > n ? "feedback-container fd-small" : "feedback-container", "var-init"),
      o = cc11001100_hook("o", l(A, e), "var-init");
    return ['<div id="feedback-container" class="' + a + '">', '<div id="reason-content" class="reason-content" data-reason-num="' + o.length + '" style="width:' + n + "px;padding-top:" + t + 'px;">', '<div class="intro">反馈意见：</div>', o.join(""), '<a class="fb-btn-cancel" href="javascript:;">返回</a>', "</div>", '<div id="thx-content" class="thx-content">', "<h5>感谢您的反馈！</h5>", "<p>您的反馈已记录，我们将努力改善您的浏览体验。</p>", "</div>", "</div>"].join("");
  }
  function l(A, e) {
    cc11001100_hook("A", A, "function-parameter");
    cc11001100_hook("e", e, "function-parameter");
    function t() {
      var A = cc11001100_hook("A", parseInt(3 * Math.random(), 10), "var-init");
      return [n[A], n[3]];
    }
    var n = cc11001100_hook("n", ['<a href="javascript:;" data-value="1" class="reason-item">视觉干扰</a>', '<a href="javascript:;" data-value="2" class="reason-item">内容不宜</a>', '<a href="javascript:;" data-value="3" class="reason-item">不感兴趣</a>', '<a href="javascript:;" data-value="0" class="reason-item">其他原因</a>'], "var-init");
    return 160 > A && 106 > e ? [] : A >= 80 && e >= 106 && 160 > A && 168 > e ? t() : A >= 160 && e >= 76 && 320 > A && 106 > e ? t() : n;
  }
  function p(A, t, o) {
    cc11001100_hook("A", A, "function-parameter");
    cc11001100_hook("t", t, "function-parameter");
    cc11001100_hook("o", o, "function-parameter");
    var i = cc11001100_hook("i", document.getElementById("logo-close-btn"), "var-init"),
      s = cc11001100_hook("s", document.getElementById("feedback-container"), "var-init"),
      c = cc11001100_hook("c", document.getElementById("reason-content"), "var-init"),
      r = cc11001100_hook("r", document.getElementById("thx-content"), "var-init");
    n(i, "click", function (n) {
      var o = cc11001100_hook("o", e(), "var-init");
      if (o && t) return o.onAdPlayEnd(), void v(A, 0);
      if (window.parent && window.postMessage && window.parent.postMessage && window.JSON && JSON.stringify) {
        var i = cc11001100_hook("i", {
          type: cc11001100_hook("type", 1, "object-key-init"),
          tuid: cc11001100_hook("tuid", u, "object-key-init"),
          msg: cc11001100_hook("msg", "click", "object-key-init")
        }, "var-init");
        parent.postMessage(JSON.stringify(i), "*");
      }
      return a(n);
    }), n(s, "click", function (t) {
      t = cc11001100_hook("t", t || window.event, "assign");
      var n = cc11001100_hook("n", t.target || t.srcElement, "var-init");
      if (n && n.getAttribute) return null != n.getAttribute("data-value") && (v(A, n.getAttribute("data-value")), c.style.display = cc11001100_hook("c.style.display", "none", "assign"), r.style.display = cc11001100_hook("r.style.display", "block", "assign"), setTimeout(function () {
        r.style.display = cc11001100_hook("r.style.display", "none", "assign");
      }, 2e3), setTimeout(function () {
        var A = cc11001100_hook("A", e(), "var-init");
        if (A) A.onAdPlayEnd();else if (window.parent && window.postMessage && window.parent.postMessage && window.JSON && JSON.stringify) {
          var t = cc11001100_hook("t", {
            type: cc11001100_hook("type", 1, "object-key-init"),
            tuid: cc11001100_hook("tuid", u, "object-key-init"),
            msg: cc11001100_hook("msg", "click", "object-key-init")
          }, "var-init");
          parent.postMessage(JSON.stringify(t), "*");
        }
      }, 2e3)), "fb-btn-cancel" === n.className && (s.style.display = cc11001100_hook("s.style.display", "none", "assign")), a(t);
    });
  }
  function v(e, t) {
    cc11001100_hook("e", e, "function-parameter");
    cc11001100_hook("t", t, "function-parameter");
    var n = cc11001100_hook("n", "https://eclick.baidu.com/close_feedback.jpg?", "var-init");
    e = cc11001100_hook("e", e || {}, "assign"), e.reasonid = cc11001100_hook("e.reasonid", t, "assign"), e._ = cc11001100_hook("e._", new Date().getTime(), "assign"), n += cc11001100_hook("n", o(e), "assign");
    var a = cc11001100_hook("a", document.createElement("img"), "var-init");
    a.onerror = cc11001100_hook("a.onerror", A, "assign"), a.src = cc11001100_hook("a.src", n, "assign");
  }
  function b() {
    if (window.parent && window.postMessage && window.parent.postMessage) {
      var A = function (A) {
        try {
          var e = cc11001100_hook("e", JSON.parse(A.data), "var-init");
          if (e && 1 === e.type) {
            u = cc11001100_hook("u", e.tuid, "assign");
            var t = cc11001100_hook("t", {
              type: cc11001100_hook("type", 1, "object-key-init"),
              msg: cc11001100_hook("msg", "ok", "object-key-init"),
              tuid: cc11001100_hook("tuid", u, "object-key-init")
            }, "var-init");
            window.parent.postMessage(JSON.stringify(t), "*");
          }
        } catch (n) {}
      };
      window.addEventListener ? window.addEventListener("message", A, !1) : window.attachEvent && window.attachEvent("onmessage", A);
    }
  }
  var u = cc11001100_hook("u", "", "var-init");
  return {
    init: cc11001100_hook("init", c, "object-key-init")
  };
});