  ...
  babel.traverse(ast, {
    ...
    VariableDeclaration(path) {
      ...
      for (const variableDeclarator of node.declarations) {
        ...
        let varName = '';
        if (
          types.isIdentifier(variableDeclarator.id) ||
          types.isMemberExpression(variableDeclarator.id)
        ) {
        ...
        try {
          ...
          variableDeclarator.init = types.callExpression(
            types.identifier(hookFunctionName),
            hookFunctionArguments
          );
        } catch (e) {
          console.error(e);
        }
    ...
    AssignmentExpression(path) {
      ...
      try {
        ...
        node.right = types.callExpression(
          types.identifier(hookFunctionName),
          hookFunctionArguments
        );
      } catch (e) {
    ...
    ObjectExpression(path) {
      ...
      for (const objectProperty of node.properties) {
        ...
        try {
          ...
          objectProperty.value = types.callExpression(
            types.identifier(hookFunctionName),
            hookFunctionArguments
          );
        } catch (e) {
      ...
      if (types.isBlockStatement(node.body)) {
        ...
        for (let i = params.length - 1; i >= 0; i--) {
          try {
            ...
            const hookFunction = types.callExpression(
              types.identifier(hookFunctionName),
              hookFunctionArguments
            );
