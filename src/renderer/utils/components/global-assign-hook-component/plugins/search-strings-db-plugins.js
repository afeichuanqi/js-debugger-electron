(() => {
  ...
  const { cc11001100_hook } = window;
  const { stringsDB } = cc11001100_hook;
  ...
  window.addEventListener('message', (event) => {
    ...
    if (eventData.type === messageTypeSearch) {
      const { pattern } = eventData;
      const { isEquals } = eventData;
      const { fieldName } = eventData;
      const { isNeedExpansion } = eventData;
      _search(fieldName, pattern, isEquals, isNeedExpansion);
      alreadyProcessMessageIdSet.add(messageId);
      _searchParentAndChildren(
        messageId,
        fieldName,
        pattern,
        isEquals,
  ...
  window.search =
    ...
    cc11001100_hook.search =
    cc11001100_hook.searchByValue =
      function (pattern, isEquals = true, isNeedExpansion = true) {
  ...
  window.searchByName = cc11001100_hook.searchByName = function (
    pattern,
    isEquals = false,
  ...
  function _searchParentAndChildren(
    messageId,
    fieldName,
    pattern,
    isEquals,
      ...
      if (typeof pattern === 'string') {
        if (isEquals) {
          ...
          for (const newPattern of expansionValues) {
            isMatch =
              isMatch ||
              (s[filedName] && s[filedName].indexOf(newPattern) !== -1);
          }
        }
  ...
  function showResult(result) {
    ...
    console.log(message);
    console.log(
      `变量名\t\t\t\t\t变量值\t\t\t\t\t变量类型\t\t\t\t\t所在函数\t\t\t\t\t执行次数\t\t\t\t\t执行顺序\t\t\t\t\t代码位置\n\n\n`
    );
    for (const s of result) {
      if (s.value.length > 90) {
        console.log(`${s.name}\t\t\t\t\t${s.value}`);
        console.log(
          `${blank(s.name.length)}\t\t\t\t\t${s.type}\t\t\t\t\t${s.codeName}`
        );
        console.log(
          `${blank(s.name.length)}\t\t\t\t\t${s.execTimes}\t\t\t\t\t${
            s.execOrder
          }`
        );
      } else {
        console.log(
          `${s.name}\t\t\t\t\t${s.value}\t\t\t\t\t${s.type}\t\t\t\t\t${s.codeName}`
        );
        console.log(
          `${blank(s.name.length)}\t\t\t\t\t${s.execTimes}\t\t\t\t\t${
            s.execOrder
